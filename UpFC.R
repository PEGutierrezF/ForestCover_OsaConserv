...
taxa <- select(UpFV.frm, Acari:Polyplectopus)
set.seed(1)
...
ordihull(taxa.mds,group=env$UpFC,show.groups="First",lty=5,lwd=2, col="grey40")
ordihull(taxa.mds,group=env$UpFC,show.groups="Second",lty=5,lwd=2, col="red3")
ordihull(taxa.mds,group=env$UpFC,show.groups="Third",lty=5,lwd=2, col="blue")
ordihull(taxa.mds,group=env$UpFC,show.groups="Fourth",lty=5,lwd=2, col="green")


points(taxa.mds, display = "sites", cex=1.5, 
       select=which(env$UpFC=="First"),
       pch = 21,col="black", bg="gray45")

points(taxa.mds, display = "sites", cex=1.5, 
       select=which(env$UpFC=="Second"),
       pch = 22,col="black", bg="red")

points(taxa.mds, display = "sites", cex=1.5, 
       select=which(env$UpFC=="Third"),
       pch = 22,col="black", bg="blue")

points(taxa.mds, display = "sites", cex=1.5, 
       select=which(env$UpFC=="Fourth"),
